...
class CodeWriter:
    ...
    def compare_logic(self, command, lines_to_write):
        ...
        lines_to_write.extend([
            "@R13",  # Store y in R13
            "M=D",
            f"@Y_NEG_{command.upper()}_{CodeWriter.counter}",
            "D;JLT",  # Jump if y < 0
            # Pop x (first operand) into D
            "@SP",
            "AM=M-1",
            "D=M",
            f"@Y_POS_X_NEG_{command.upper()}_{CodeWriter.counter}",
            "D;JLT",  # Jump if x < 0
            "@R13",
            "D=D-M",
            f"@CHECK_{command.upper()}_{CodeWriter.counter}",
            "0;JMP",  # Jump if x and y have different signs
            f"(Y_NEG_{command.upper()}_{CodeWriter.counter})",  # Handle y < 0
            "@SP",
            "AM=M-1",
            "D=M",
            f"@Y_NEG_X_POS_{command.upper()}_{CodeWriter.counter}",
            "D;JGT",  # Jump if x > 0
            "@R13",
            "D=D-M",
            f"@CHECK_{command.upper()}_{CodeWriter.counter}",
            "0;JMP",  # Jump if x and y have different signs
            f"(Y_POS_X_NEG_{command.upper()}_{CodeWriter.counter})",  # Handle x >= 0, y < 0
            "D=-1",
            f"@CHECK_{command.upper()}_{CodeWriter.counter}",
            "0;JMP",
            f"(Y_NEG_X_POS_{command.upper()}_{CodeWriter.counter})",  # Handle x < 0, y >= 0
            "D=1",
            f"@CHECK_{command.upper()}_{CodeWriter.counter}",
            "0;JMP",
            f"(CHECK_{command.upper()}_{CodeWriter.counter})",
            f"@TRUE_{command.upper()}_{CodeWriter.counter}",
            self.ARITHMETIC_DICT[command],
            "D=0",
            f"@CONTINUE_{command.upper()}_{CodeWriter.counter}",
            "0;JMP",
            f"(TRUE_{command.upper()}_{CodeWriter.counter})",
            "D=-1",
            f"(CONTINUE_{command.upper()}_{CodeWriter.counter})",
